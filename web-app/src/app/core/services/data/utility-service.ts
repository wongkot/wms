...
export class UtilityService {
  public formatAreaName(zonePrefix: string, row: number, column: number): string {
    return `${zonePrefix}-${row}${column}`;
  }

  public formatLotNumber(date: Date): string {
    return formatDate(date, 'yyyyMMdd', DEFAULT_LOCALE);
  }

  public sortArray<T = any>(array: T[], sortColumn: any, isAsc: boolean): T[] {
    if (!array) {
      return array;
    }
  ...
  public getProductCategoriesForDropdown(isFilter: boolean = true): KeyValue<string, string>[] {
    let dropDownList = [
  ...
  public getAreasForDropdown(): KeyValue<string, string>[] {
