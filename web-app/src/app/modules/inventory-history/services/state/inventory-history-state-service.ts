...
export class InventoryHistoryStateService {
  ...
  loadInventoryHistories(options?: { page?: number, pageSize?: number, query?: string, operationType?: number | null, sort?: TableSortState }): void {
    ...
    this._inventoryHistoryService.getInventoryHistories(page, pageSize, query, operationType, `${sort?.columnProp}:${sort?.isAsc ? 'asc' : 'desc'}`).pipe(
