...
export class MockInventoryHistoryService implements InventoryHistoryService {
  ...
  public getInventoryHistories(page: number, pageSize: number, query: string, operationType: number | null, startDate: string, endDate: string, sort: string): Observable<Pagination<InventoryHistory>> {
    return of(this._inMemoryDbService.getPageInventoryHistories(page, pageSize, query, operationType, startDate, endDate, sort));
  }

  public getRecentInventoryHistories(limit: number): Observable<InventoryHistory[]> {
