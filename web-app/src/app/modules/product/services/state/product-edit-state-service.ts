...
export class ProductEditStateService {
  ...
  private readonly _initialImageUrl = signal<string>('');
  private readonly _productService: ProductService = inject(MockProductService);
  private readonly _productLoadSuccess = new Subject<Product>();
  private readonly _editSuccess = new Subject<void>();
  private readonly _editFailed = new Subject<string>();
  ...
  public get initialImageUrl() {
    return this._initialImageUrl.asReadonly();
  }

  public loadProductInfo(productId: number): void {
  ...
  public editProduct(input: EditProduct): void {
  ...
  public isProductNameExistsValidator(productId: number): AsyncValidatorFn {
    return control => {
